...
public class NotificationsIT {
  ...
  @ClassRule
  public static final DropwizardAppRule<NotificationConfiguration> RULE =
      new DropwizardAppRule<NotificationConfiguration>(NotificationApplication.class,
  ...
  public void testCreateNotification() throws Exception {
    ...
    final Response response = client.target(getUrl()).request().post(Entity.json(notification));

    final Notification actual = response.readEntity(Notification.class);

    assertThat(response.getStatus()).isEqualTo(201);
    assertThat(response.getLocation().getPath()).isEqualTo("/v1/notifications/" + TEST_USER);
  ...
  public void testPagination() throws Exception {
    ...
    for (int i = 0; i < 100; i++) {
      final Notification response =
          client.target(getUrl()).request()
              .post(Entity.json(createNotification()), Notification.class);
      expected.add(Notification.builder().fromNotification(response).withUnseen(true).build());
    }

    int requests = 0;
    String nextRange = null;
    final ImmutableList.Builder<Notification> results = ImmutableList.builder();
    boolean paginate = true;
    while (paginate) {
      final Invocation.Builder builder = client.target(getUrl()).request();
      if (nextRange != null) {
  ...
  private static Notification createNotification() {
    return Notification.builder().withCategory("test-category").withMessage("this is only a test")
        .build();
  }
}
