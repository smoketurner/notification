...
public class NotificationListResolver implements ConflictResolver<NotificationListObject> {

  private static final Logger LOGGER = LoggerFactory.getLogger(NotificationListResolver.class);
  ...
  public NotificationListObject resolve(final List<NotificationListObject> siblings)
    ...
    if (siblings.size() > 1) {
      ...
      final NotificationListObject resolved = new NotificationListObject(iterator.next());
      ...
      if (!deletedIds.isEmpty()) {
        ...
        resolved.setNotifications(removeNotifications(resolved.getNotifications(), deletedIds));
      ...
      if (!resolved.getDeletedIds().isEmpty()) {
        ...
        resolved.setNotifications(removeNotifications(resolved.getNotifications(),
  ...
  /**
   * Remove the given notification IDs from the list of notifications. We have to copy the iterable
   * into a list to avoid modifying the original reference within the {@link NotificationListObject}
   * .
   * 
   ...
   */
  public static List<Notification> removeNotifications(final Iterable<Notification> notifications,
      final Collection<Long> ids) {

    return ImmutableList.copyOf(Iterables.filter(notifications, new Predicate<Notification>() {
      @Override
